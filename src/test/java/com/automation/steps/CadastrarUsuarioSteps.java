...
public class CadastrarUsuarioSteps {
    ...
    @Given("o request da API CADASTRAR USUARIO {string}")
    public void o_request_da_API_CADASTRAR_USUARIO(String requestBody){
        this.body = JsonUtils.getFileContentFromFullPath(configuration.bodyPath().concat(requestBody));
        scenario.log(body);
    }

    
    @When("o sistema envia um request POST na API CADASTRAR USUARIO {string}")
    public void o_sistema_envia_um_request_POST_na_API_CADASTRAR_USUARIO(String apiName) {
        response = resource.postRequest(apiName, body);
        specs = new PropostaSpecs(response);
        scenario.log(response.asPrettyString());
    }
    
    @Then("o status code da API CADASTRAR USUARIO retorna {int}")
    public void o_status_code_da_API_CADASTRAR_USUARIO_retorna(final Integer statusCodeEsperado) {
        specs.verifyResponseStatusValue(statusCodeEsperado);
        statusCode = this.response.getStatusCode();
        scenario.log(statusCode.toString());
    }

    @And("o response body da API CADASTRAR USUARIO confere com o JSON {string}")
    public void o_response_body_da_API_CADASTRAR_USUARIO_confere_com_o_JSON(String schemaName) {
